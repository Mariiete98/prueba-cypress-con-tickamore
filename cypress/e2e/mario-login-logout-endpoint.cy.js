...
describe(LoginData.titulos.loginAPI, () => {
    ...
    it('TEST 4.1. Aceptar Cookies predeterminadas y abrir Login correcto con endpoint. Status 200.', () => {
            ...
            cy.wait('@todasLasPost').then((interception) => {
                ...
                if (statusC === 200 && contenido.datosResult !== null) {
                    ...
                    LoginFunciones.verificarDesplegableAbierto(LoginElementos.login.desplegableUsuario);
    ...
    it('TEST 4.2. Aceptar Cookies predeterminadas, abrir Login correcto con endpoint. Cerrar Sesión con endpoint. Status 200.', () => {
            ...
            cy.wait('@todasLasPost').then((interception) => {
                ...
                const status1 = interception.response.statusCode;
                const contenidoRespuesta1 = interception.response.body;

                // pruebas de respuesta
                Logger.subVerificacion('CODIGO RECUPERADO: ', status1);
                Logger.subVerificacion('BODY RECUPERADO: ', contenidoRespuesta1);


                Logger.subPaso('Si fue correcto el Login, devuelve 200. Sino, se cierra el Login');
                //el error tambien devuelve 200, hay que comparar el body si es null
                if (status1 === 200 && contenidoRespuesta1.datosResult !== null) {

                    Logger.subVerificacion('Login Correcto. Comprobar que se obtiene el usuario que carga en botón del menú');
                    // donde se ve?
                    Logger.subVerificacion('Usuario: ', contenidoRespuesta1.datosResult.clienteNombre); // DatosResult es Objeto
                    ...
                    Logger.paso('Cerrar sesión')
                    cy.intercept('POST', endpointLogout).as('apiLogout');

                        Logger.subPaso('Click en Salir. Comprobar la no lectura del desplegable.')
                        LoginFunciones.clickElemento(LoginElementos.login.botonSalir);
                        cy.wait(2000);
                        LoginFunciones.verificarDesplegableCerrado(LoginElementos.login.desplegableUsuario);
                        Logger.subVerificacion('Cierre de sesión correcto')

                    cy.wait('@apiLogout').then((interception) => {

                        const status2 = interception.response.statusCode;
                        const contenidoRespuesta2 = interception.response.body;

                        Logger.subPaso('Si fue correcto el cierre de sesión, desaparece el desplegable');
                        // pruebas de respuesta
                        Logger.subVerificacion('CODIGO RECUPERADO: ', status2);
                        Logger.subVerificacion('BODY RECUPERADO: ', contenidoRespuesta2);

                        if (status2 === 200 && contenidoRespuesta2.datosResult != null) {
                            ...
                            Logger.subVerificacion('No se ha podido cerrar sesión')
                            Logger.error('Error: ', contenidoRespuesta2.mensajes[0].descripcionMensaje);
    ...
    it('TEST 4.3. Aceptar Cookies predeterminadas y probar Login incorrecto con endpoint. Error. Status 200 pero body null, por ambas credenciales incorrectas', () => {
